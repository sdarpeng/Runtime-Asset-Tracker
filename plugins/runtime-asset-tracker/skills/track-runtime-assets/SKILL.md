...
Discovery is deliberately broad; execution is deliberately narrow. Do not depend on a human-maintained `disposable` tag to discover likely waste. Correlate exact image IDs, repository-scoped build families, timestamps, build success/failure events, runtime consumers, Git revision/source, merged-PR lifecycle, and disk pressure. Classify every finding as `suspected-retired`, `blocked-candidate`, or `executable-candidate`. A missing recovery source, unproven successor success, cooling period, live or stopped reference, current/rollback/recovery identity, remote automatic candidate, or unattested volume must be an explicit blocker instead of making the asset invisible. Capacity pressure expands and prioritizes the candidate inventory; it never removes blockers or bypasses exact confirmation.

Remote cleanup always starts with an exact preview. For EC2, safely executable images are limited to images that are not referenced by any running or stopped container and are either dangling or covered by exact retirement evidence; protected retention always wins. Safely executable volumes must be unreferenced by every container, explicitly attested disposable, and free of database, upload, media, backup, or other protected-state signals. Unused Docker BuildKit cache is also eligible. Exact merged-PR containers may be stopped and removed only when their full ID, name, image ID, Compose project, state, mount set, recovery source, completed cooling period, and `preserveVolumes=true` contract are attested. Exact remote paths may be removed only below a registered managed root with matching bytes and metadata fingerprint, zero bind-mount consumers, and no current/rollback/protected-path overlap. Networks, rollback state, unmanaged directories, and unknown volumes remain excluded. For GitHub, cleanup is limited to exact cache or artifact IDs that are independently reclassified as safe immediately before deletion: expired artifacts, caches belonging to closed pull requests, or caches not accessed for more than 30 days. A disconnected source remains unavailable instead of falling back to local data.

For local standalone preview, run `node dist/server.mjs --http` from the plugin root and open the one-time authenticated URL printed by the process. The dashboard is loopback-only; destructive HTTP routes require the session token and same-origin request.
...
5. Call `preview_cleanup` to create the exact expiring allowlist. Only `executable-candidate` assets enter it; `suspected-retired` and `blocked-candidate` remain visible with their missing dependencies. Use `assetIds` when the authorized scope is an exact subset.
...
10. Local path cleanup is executable only when an exact retirement attestation still matches the live bytes and fingerprint. On POSIX, use the bundled `dir_fd`/`O_NOFOLLOW` helper. On Windows, use the provenance-bound native helper, which anchors deletion to a verified managed-root handle, opens every descendant relative to its parent with `FILE_OPEN_REPARSE_POINT`, and binds the root/target volume serial and file ID before mutation. Reparse descendants may be removed as links but are never traversed. An exact clean registered worktree may be retired only when its checkout `.git` file, common Git directory, one exact `.git/worktrees/<id>` metadata directory, and backlink form a bidirectional contract; lock the worktree, remove the checkout and that exact metadata directory through the same handle-safe helper, and keep primary, dirty, explicitly locked, or ambiguous worktrees blocked.
...
- `../../scripts/safe-delete-path.py`: POSIX descriptor-relative path cleanup helper.
- `../../scripts/safe-delete-path-windows.ps1`: Windows native handle-relative path cleanup helper.
